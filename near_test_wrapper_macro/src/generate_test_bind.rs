...
pub(crate) fn generate_view_function(func_info: FunctionInfo) -> TokenStream {
    ...
    let return_ident = quote! { -> near_test_wrapper_types::ImmutablePendingTx };

    let func = quote! {
        #[cfg(not(all(target_arch = "wasm32", target_os = "unknown")))]
        pub fn #name(&self, #params) #return_ident {
            #serialize_args
            near_test_wrapper_types::ImmutablePendingTx::new( String::from(#name_str), args)
        }
    };
...
pub(crate) fn generate_call_function(func_info: FunctionInfo) -> TokenStream {
    ...
    let name_str = func_info.function_name.to_string();
    let params = func_info.params.clone();
    let return_ident = quote! { -> near_test_wrapper_types::MutablePendingTx };

    let func = quote! {
        pub fn #name(&self, #params) #return_ident {
            #serialize_args
            near_test_wrapper_types::MutablePendingTx::new( String::from(#name_str), args)
        }
    };
...
pub(crate) fn generate_payable_function(func_info: FunctionInfo) -> TokenStream {
    ...
    let name_str = func_info.function_name.to_string();
    let params = func_info.params.clone();
    let return_ident = quote! { -> near_test_wrapper_types::PayablePendingTx };
    ...
    let func = quote! {
        pub fn #name(&self, #params #deposit_ident) #return_ident {
            #serialize_args
            near_test_wrapper_types::PayablePendingTx::new( String::from(#name_str), args, attached_deposit)
        }
    };
